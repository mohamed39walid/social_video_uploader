...
class VideoPostAdmin(admin.ModelAdmin):
    ...
    list_filter = ("platforms", "created_at")
    readonly_fields = (
        "youtube_link",
        "vimeo_link",
        "dailymotion_link",
    ...
    def youtube_link(self, obj):
        if obj.youtube_video_id:
            ...
            return format_html('<a href="{}" target="_blank">{}</a>', url, obj.youtube_video_id)
        return "-"
    ...
    def upload_to_platforms(self, request, queryset):
        for video in queryset:
            ...
            if "VM" in video.platforms:
                try:
                    # TODO: integrate Vimeo API
                    video.vimeo_video_id = f"DEBUG_VIMEO_{video.id}"
                    upload_status["VM"] = "uploaded"
                    self.message_user(request, f"Simulated Vimeo upload for {video.title}")
                except Exception as e:
